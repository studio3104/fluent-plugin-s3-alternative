...
class S3OutputTest < Test::Unit::TestCase
  ...
  def create_driver(conf = CONFIG)
    Fluent::Test::BufferedOutputTestDriver.new(Fluent::S3AlternativeOutput) do
      def write(chunk)
        chunk.read
      end
    end.configure(conf)
...
#  end
  ...
  def test_write_with_custom_s3_object_key_format
    ...
    d = Fluent::Test::TimeSlicedOutputTestDriver.new(Fluent::S3AlternativeOutput).configure(%[
      aws_key_id test_key_id
      aws_sec_key test_sec_key
      s3_bucket test_bucket
